...
\simon{Need to first define the DKG challenge $\dkgchal$ and to define $\simid$.}
...
C[0]_{\simid} \gets \dkgchal \cdot
        ...
        \item Generate a simulated zero-knowledge proof $\pi_\simid$ of $\SchnorrR$ for $C[0]_\simid$.
            ...
            \simon{i.e., by picking the Schnorr commitment/response pair $(c, s)$ at random, computing~ $R\gets g^s/\PK_{\mathsf{Schnorr}}^c$ then reprogramming
            the random oracle $\HashSchnorrR(R,\gamma_\simid)\gets c$.}
        \item\simon{Reprogram the random oracle $\HashCom(\sid, k, C_k)\gets H_k$} 
    ...
    \begin{itemize}
      ...
      \item
      \simon{
        \item Otherwise, for each corrupt party $j \in \corrupt$ and $k\in \honest$, look up $f_j(k) \gets \QSchnorrR[H_j]$.
