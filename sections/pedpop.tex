...
We depict $\pedpop$ in Figure~\ref{fig:pedpop} and refer the reader to~\cite{KomGol2021} for proof details.
...
\quad f(j) \gets \alpha + \sum_{i=1}^{\tcor} a_i j^i \\
...
\pcfor i \in \set{n}, i \neq k \pcdo \\
  ...
  \quad \require \schnorr[\Hash_1].\Verify(C_i[0], \Phi, \pi_i) = 1 \\
  ...
  \quad \pk_i \gets \prod_{j=1}^n \prod_{\ell=0}^{\tcor} C_j[\ell]^{i^\ell}\\
...
$\Phi$ is a context string that aims to prevent replay attacks. It contains the identifier of the signer and the verification key share.
}
\label{fig:pedpop}
\end{figure}
