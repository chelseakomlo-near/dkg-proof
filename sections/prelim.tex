...
\subsection{Online-Extractable \& Straight-Line Simulation Proof System}
...
\textbf{KeyGen:} On query with input identifier $\text{id}'$ from party $\text{id}$.
\begin{enumerate}
    \item Output $\perp$ if $\text{id}'$ is not the identifier for party $\text{id}$.
    \item Output $\perp$ if $Q_K^{(1)}[\text{id}] \neq \perp$.
    \item Else, generate $\textsf{sk} \xleftarrow{\$} \mathbb{Z}_s$ and $\textsf{PK} \leftarrow g^{\textsf{sk}}$.
    \item Set $Q_K^{(1)} \leftarrow Q_K[\text{id}] = \textsf{PK}$.
    \item Output $(\textsf{sk}, \textsf{PK})$.
\end{enumerate}

\textbf{Register:} On query with public key $\textsf{PK}$, secret key $\textsf{sk}$, and identifier $\text{id}'$ from party $\text{id}$.
\begin{enumerate}
    \item Output $\perp$ if $\text{id}'$ is not the identifier for party $\text{id}$.
    \item Output $\perp$ if $Q_K^{(2)}[\text{id}] \neq \perp$.
    \item Output $\perp$ if $(\textsf{PK}, \textsf{sk})$ do not satisfy the relation $R_{\text{pki}}$, where
    \[
    \mathcal{F}^{R_{EF}}_{zk} = \{(g, \textsf{PK}, (\textsf{sk})) : \textsf{PK} = g^{\textsf{sk}}\}.
    \]
    \item Else, set $Q_K^{(2)}[\text{id}] = \textsf{PK}$.
\end{enumerate}
...
\caption{Ideal functionality $\mathcal{F}^{R_{EF}}_{zk}$ for a public-key infrastructure with proof of possession of the corresponding secret key $\textsf{sk}$. Public parameters $\textsf{par}$ are given implicitly to all algorithms. The functionality authenticates participant queries, ensuring that parties can only generate and register keys for themselves.}
\end{figure}




Proving the security of our protocol requires realizing $\mathcal{F}^{R_{EF}}_{zk}$ which in our case requires
...
\end{align*}
\simon{How to add the session identifier in the relation?}

A practical construction that we will be using is as follows:
...
\item \textbf{KeyGen:} If $Q_K^{(1)}[\mathsf{id}] \neq \perp$, return $\perp$.  
Else, sample $\mathsf{sk} \leftarrow_{\$} \mathbb{Z}_s$, set $\mathsf{PK} \leftarrow g^{\mathsf{sk}}$.  
Set $Q_K^{(1)}[\mathsf{id}] = \mathsf{PK}$. Output $(\mathsf{PK}, \mathsf{sk})$.

\item \textbf{Register:} Accepts as input public key $\mathsf{PK}$, identifier $\mathsf{id}'$ from party $\mathsf{id}$, and proof $\pi$.
\begin{enumerate}
    \item Output $\perp$ if $\mathsf{id}'$ is not the identifier for party $\mathsf{id}$.
    \item Output $\perp$ if $Q_K^{(2)}[\mathsf{id}] \neq \perp$.
...
\begin{enumerate}
\setcounter{enumi}{3}
    \item Else, set $Q_K^{(2)}[\mathsf{id}] = \mathsf{PK}$.
\end{enumerate}
...
\end{itemize}
    ...
    \end{center}
  \end{framed}
    \caption{Concrete protocol realizing $\mathcal{F}^{R_{EF}}_{zk}$.
    }
  \label{fg:keygen-abort}
\end{figure}
