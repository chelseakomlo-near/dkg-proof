...
$$
  \mathcal{R}_P = \left\{ \left(g, h, \PK, c = (C_0, C_1), z, R_g, R_h \right) ; (\mathsf{sk}, r) \right\} \quad \text{iff}\\
$$
\begin{align*}
  &(1) \quad C_0 = g^r; \quad C_1 = h^r \cdot g^{\mathsf{sk}} \\
  &(2) \quad g^z = R_g \cdot \left(g^{\mathsf{sk}}\right)^{c'} \\
  &(3) \quad h^z = R_h \cdot \left(h^{\mathsf{sk}}\right)^{c'}
\end{align*}
...
The statement consists of: the generator $g$ of $\Gr$ and the random group element $h \rgets \Gr$ fixed at setup;
the public key $\PK = g^{\sk}$; the ElGamal ciphertext $c = (C_0, C_1)$ of $g^{\sk}$ under randomness $r$;
and the Schnorr proof transcript $(R_g, R_h, z)$, where $R_g, R_h$ are the prover's commitments and $z$ is the response.
...
\[
  c' = \HashSchnorrR(R_g, R_h, \PK, \mathsf{transcript}),
\]
where the hash binds the commitments $R_g, R_h$, the statement $\PK$, and the remaining transcript elements
(the public values $g, h$, the ciphertext $c$, and the proof context $\gamma$, e.g.\@ $\gamma = (\sid, k, C_k[0])$ in the protocol).

\simon{Verify the need of (1)}
...
\mathcal{R}_P =
\left\{
\big( g, h, \mathsf{PK}, c = (C_0, C_1), z, R_g, R_h \big) ; (\mathsf{sk}, r)
\right\}
\quad \text{iff}
\]
...
\begin{align*}
(1)\quad & C_0 = g^r; \quad C_1 = h^r \cdot g^{\mathsf{sk}} \\
(2)\quad & g^z = R_g \cdot (g^{\mathsf{sk}})^{c'} \\
(3)\quad & h^z = R_h \cdot (h^{\mathsf{sk}})^{c'}
\end{align*}

Here the witness is $(\sk, r)$; the statement comprises the generator $g$, the setup element $h \rgets \Gr$,
the public key $\PK = g^{\sk}$, the ElGamal ciphertext $c = (C_0, C_1)$, and the proof transcript $(R_g, R_h, z)$
with commitments $R_g, R_h$ and response $z$.
The challenge $c'$ is not part of the proof: the verifier recomputes it directly via Fiat--Shamir as
\[
  c' = \HashSchnorrR(R_g, R_h, \PK, \mathsf{transcript}),
\]
binding the commitments $R_g, R_h$, the statement $\PK$, and the remaining transcript elements
($g, h$, the ciphertext $c$, and the proof context).
