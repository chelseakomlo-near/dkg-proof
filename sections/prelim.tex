...
Proving the security of our protocol requires a zero-knowledge proof system
that must be online-extractable as in the UC framework, rewinding is not allowed.

One possible direction is to use zero-knowledge proofs described in \cite{GOLDEN}.
The zero-knowledge proof relation $\mathcal{R}_P$, described in the following relation
...
\simon{Ideal functionality can be found in \cite{GOLDEN}}
...
\end{itemize}
    ...
    \end{center}
  \end{framed}
    \caption{Concrete protocol realizing ideal functionality for protocol.
    }
  \label{fg:keygen-abort}
\end{figure}
