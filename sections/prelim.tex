...
\caption{Ideal functionality $\mathcal{F}^{R_{EF}}_{zk}$ for a public-key infrastructure with proof of possession of the corresponding secret key $\textsf{sk}$. Public parameters $\textsf{par}$ are given implicitly to all algorithms.}
\end{figure}
...
\begin{enumerate}
    \item Output $\perp$ if $i'$ is not the identifier for party $i$.
